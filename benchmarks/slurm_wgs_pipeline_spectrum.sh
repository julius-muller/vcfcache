...
run_root=/mnt/data/tmp/vcfcache_wgs_pipeline_runs/$campaign_id/task-$task_id
...
for mode in "${modes[@]}"; do
  srun --exclusive --nodes=1 --ntasks=1 --cpus-per-task=8 \
    ...
    --run-root "$run_root" --mode "$mode"
done
srun --exclusive --nodes=1 --ntasks=1 --cpus-per-task=8 \
  "$repo_root/.venv/bin/python" \
  "$repo_root/benchmarks/run_wgs_pipeline_spectrum_task.py" \
  --task-manifest "$task_manifest" --task-id "$task_id" \
  --run-root "$run_root" --mode compare
test -s "$run_root/wgs_pipeline_summary.json"
