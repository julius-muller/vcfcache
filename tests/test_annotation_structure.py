"""Test the structure and content of VCFstash annotations.

This file combines tests from the previous test_annotation_results.py and 
test_cache_annotate.py files, focusing on the structure and content of 
annotation outputs rather than the annotation process itself.
"""
...
def fix_blueprint_snapshot(stash_dir, anno_name):
...
def test_annotation_directory_structure(annotated_stash):
    """Test that the annotation directory has the expected structure."""
    stash_dir, anno_name = annotated_stash

    # Verify annotation directory structure
    annotation_dir = os.path.join(stash_dir, "stash", anno_name)
    ...
    expected_files = [
        ...
        f"{anno_name}_flowchart.html",  # Nextflow flowchart
        f"{anno_name}_report.html",  # Nextflow report
        f"{anno_name}_trace.txt",  # Nextflow trace
    ...
    # Check for annotation.yaml in the stash directory
    stash_dir_path = os.path.join(stash_dir, "stash")
    assert os.path.exists(os.path.join(stash_dir_path, "annotation.yaml")), "annotation.yaml not found in stash directory"

    # Check for workflow directory
    workflow_dir = os.path.join(stash_dir, "workflow")
    ...
    print(f"Directory structure of {stash_dir}:")
    for root, dirs, files in os.walk(stash_dir):
        rel_path = os.path.relpath(root, stash_dir)
        if rel_path == '.':
            print(f"  - {', '.join(dirs)} (dirs)")
            print(f"  - {', '.join(files)} (files)")
        else:
            print(f"  - {rel_path}/")
            print(f"    - {', '.join(dirs)} (dirs)")
            print(f"    - {', '.join(files)} (files)")