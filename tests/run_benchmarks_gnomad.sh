...
for AF in "${AF_VALUES[@]}"; do
    ...
    if [ ! -d "${STASH_DIR}" ]; then
      ...
      /home/appuser/projects/vepstash/vepstash.py stash-init $VERBOSE \
        --vcf /mnt/data/resources/gnomad/vep_gnomad_v4_hg19_exomes/gnomad.exomes.v4.1.sites.grch37.trimmed_liftover_norm_${AF}.bcf \
        --output ${STASH_DIR} \
        -c /home/appuser/projects/vepstash/tests/config/nextflow_gnomadhg19.config
    else
      echo "Skipping stash-init for ${AF} as ${STASH_DIR} exists."
    fi

    # --- Step 2: First Annotation (gnomad_ex) ---
    if [ ! -d "${STASH_DIR}/annotations/gnomad_ex_${AF}" ]; then
      echo "Running stash-annotate (gnomad_ex_${AF})"
      /home/appuser/projects/vepstash/vepstash.py stash-annotate $VERBOSE \
        --name gnomad_ex_${AF} \
        -a /home/appuser/projects/vepstash/tests/config/annotation.config \
        --db ${STASH_DIR}
    else
      echo "Skipping stash-annotate (gnomad_ex_${AF}) as ${STASH_DIR}/annotations/gnomad_ex_${AF} exists."
    fi

    # --- Step 3: Add Genomes Dataset ---
    if [ ! -f "${STASH_DIR}/genomes_added.flag" ]; then
      echo "Running stash-add (genomes dataset) for ${AF}"
      /home/appuser/projects/vepstash/vepstash.py stash-add $VERBOSE \
        --db ${STASH_DIR} \
        -i /mnt/data/resources/gnomad/vep_gnomad_v4_hg19_genomes/gnomad.genomes.v4.1.sites.grch37.trimmed_liftover_norm_${AF}.bcf
    ...
    if [ ! -d "${STASH_DIR}/annotations/gnomad_genex_${AF}" ]; then
      ...
      echo "Skipping stash-annotate (gnomad_genex_${AF}) as ${STASH_DIR}/annotations/gnomad_genex_${AF} exists."
    fi
    ...
    if [ ! -d "${STASH_DIR}/annotations/gnomad_genexd_${AF}" ]; then
      ...
      echo "Skipping stash-annotate (gnomad_genexd_${AF}) as ${STASH_DIR}/annotations/gnomad_genexd_${AF} exists."
    fi
    ...
    for CV in "${CV_VALUES[@]}"; do
        ...
        ANNOTATION_DIR="${STASH_DIR}/annotations/gnomad_${CV}_${AF}"

        if [ ! -d "${OUTPUT_WITH}" ]; then
          echo "Running annotation with cache for ${CV}_${AF}"
          /home/appuser/projects/vepstash/vepstash.py annotate $VERBOSE \
            -a ${ANNOTATION_DIR} \
...
ANNOTATION_DIR_UC="/mnt/data/samples/bench/vst_caches/gnomad_genex_1e-1/annotations/gnomad_ex_1e-1"
