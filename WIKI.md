...
### Cache download location (`VCFCACHE_DIR`)

By default, resolved caches/blueprints are downloaded and extracted under:
- `~/.cache/vcfcache/caches/`
- `~/.cache/vcfcache/blueprints/`
...
This is useful for:
- shared scratch storage on clusters
- CI jobs (keep state inside the workspace)
- keeping large caches off your home filesystem
...
- `--target final` is the lean runtime image:
  ```bash
  docker build --target final -f docker/Dockerfile.vcfcache -t ghcr.io/julius-muller/vcfcache:vX.Y.Z .
  ```
